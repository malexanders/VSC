...
ClusterHashSlots = 16_384
MigrateDefaultTimeout = 60_000
  ...
  def initialize(addr)
              ...
              @info = @info.merge(info)
                ...
                slots.each do|s|
                  if s[0..0] == '['
                      ...
                      start, stop = s.split('-')
                      add_slots((start.to_i)..(stop.to_i))
                  else
                      add_slots((s.to_i)..(s.to_i))
                  end
                end if slots
                @dirty = false
    ...
    def info_string
      ...
      slots = @info[:slots].keys.sort
        ...
        slots = slots.map do|x|
          x.count == 1 ? x.first.to_s : "#{x.first}-#{x.last}"
        end.join(',')

        role = has_flag?('master') ? 'M' : 'S'

        if info[:replicate] && @dirty
          is = "S: #{info[:name]} #{self}"
        else
          is = "#{role}: #{info[:name]} #{self}\n"\
               "   slots:#{slots} (#{self.slots.length} slots) "\
               ((info[:flags] - ['myself']).join(',')).to_s
        end
        if info[:replicate]
          is += "\n   replicates #{info[:replicate]}"
        elsif has_flag?('master') && info[:replicas]
...
class RedisTrib
  def initialize
    ...
    def check_arity(req_args, num_args)
      if (req_args > 0 && num_args != req_args) ||
         (req_args < 0 && num_args < req_args.abs)
    ...
    def get_node_by_name(name)
      @nodes.each do|n|
          return n if name.casecmp(n.info[:name]).zero?
      end
        nil
    end
    ...
    def get_node_by_abbreviated_name(name)
      l = name.length
        candidates = []
        @nodes.each do|n|
          if name.casecmp(n.info[:name][0...l]).zero?
              candidates << n
          end
        end
    ...
    def show_cluster_info
      masters = 0
        ...
        keys_per_slot = sprintf('%.2f', keys / 16_384.0)
      ...
      xputs '>>> Check for open slots...'
        ...
        @nodes.each do|n|
          if n.info[:migrating].size > 0
            ...
            next unless n.info[:importing].size > 0
          cluster_error \
        ...
        open_slots.each{|slot| fix_open_slot slot} if @fix
    end
    ...
    def fix_slots_coverage
      not_covered = (0...ClusterHashSlots).to_a - covered_slots.keys
        ...
        slots = {}
        not_covered.each do|slot|
          nodes = nodes_with_keys_in_slot(slot)
            slots[slot] = nodes
            xputs "Slot #{slot} has keys in #{nodes.length} nodes: #{nodes.join(', ')}"
        end

        none = slots.select { |_k, v| v.length == 0 }
        single = slots.select { |_k, v| v.length == 1 }
        multi = slots.select { |_k, v| v.length > 1 }
    ...
    def get_node_with_most_keys_in_slot(_nodes, slot)
      best = nil
        best_numkeys = 0
        @nodes.each do|n|
          next if n.has_flag?('slave')
    ...
    def alloc_slots
      nodes_count = @nodes.length
        ...
        ips = {}
        @nodes.each do|n|
          ips[n.info[:host]] = [] unless ips[n.info[:host]]
            ips[n.info[:host]] << n
        end
        ...
        first = 0
        cursor = 0.0
        masters.each_with_index do|n, masternum|
    ...
    def join_cluster
      ...
      first = false
        @nodes.each do|n|
          unless first then first = n.info; next; end # Skip the first node
    ...
    def yes_or_die(msg)
      print "#{msg} (type 'yes' to accept): "
        STDOUT.flush
        if !(STDIN.gets.chomp.casecmp("yes").zero?)
          xputs '*** Aborting...'
            exit 1
        end
    end
    ...
    def compute_reshard_table(sources, numslots)
      moved = []
        ...
        source_tot_slots = sources.inject(0) do|sum, source|
        ...
        sources.each_with_index do|s, i|
          ...
          n = (numslots.to_f / source_tot_slots * s.slots.length)
            ...
            s.slots.keys.sort[(0...n)].each do|slot|
    ...
    def check_cluster_cmd(argv, _opt)
      load_cluster_info_from_node(argv[0])
        check_cluster
    end

    def info_cluster_cmd(argv, _opt)
      load_cluster_info_from_node(argv[0])
        show_cluster_info
    end
    ...
    def rebalance_cluster_cmd(argv, opt)
      opt = {
        ...
        while dst_idx < src_idx
          dst = sn[dst_idx]
            src = sn[src_idx]
            numslots = [dst.info[:balance], src.info[:balance]].map do|n|
              n.abs
            end.min
    ...
    def reshard_cluster_cmd(argv, opt)
      opt = { 'pipeline' => MigrateDefaultPipeline }.merge(opt)
        ...
        if opt['slots']
          ...
          numslots = 0
            while numslots <= 0 || numslots > ClusterHashSlots
        ...
        sources = []
        if opt['from']
          opt['from'].split(',').each do|node_id|
              if node_id == 'all'
                  sources = 'all'
                  break
              end
      ...
      xputs ">>> Adding node #{argv[0]} to cluster #{argv[1]}"
        ...
        if opt['slave']
          if opt['master-id']
              ...
              xputs "[ERR] No such master ID #{opt['master-id']}" if !master
          else
    ...
    def delnode_cluster_cmd(argv, _opt)
      id = argv[1].downcase
        ...
        @nodes.each do|n|
          next if n == node
            if n.info[:replicate] && n.info[:replicate].casecmp(id).zero?
    ...
    def set_timeout_cluster_cmd(argv, _opt)
      timeout = argv[1].to_i
        if timeout < 100
    ...
    def call_cluster_cmd(argv, _opt)
      cmd = argv[1..-1]
        cmd[0] = cmd[0].upcase
    ...
    def help_cluster_cmd(_argv, _opt)
      show_help
        exit 0
    end
    ...
    def parse_options(cmd)
      idx = 1; # Current index into ARGV
        ...
        if ALLOWED_OPTIONS[cmd]
          ALLOWED_OPTIONS[cmd].each do|option, val|
              next unless !options[option] && val == :required
...
def key_to_slot(key)
  ...
  s = key.index '{'
    ...
    RedisClusterCRC16.crc16(key) % 16_384
end
...
def show_help
  puts "Usage: redis-trib <command> <options> <arguments ...>\n\n"
    COMMANDS.each do|k, v|
      o = ''
        puts "  #{k.ljust(15)} #{v[2]}"
        next unless ALLOWED_OPTIONS[k]
      ALLOWED_OPTIONS[k].each do|optname, has_arg|
